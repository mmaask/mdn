...
class Modena_Onboarding_Handler
{
    ...
    public function maybe_received_credentials()
    {
        ...
        $wc_modena_admin_nonce = isset($_GET['wc_modena_admin_nonce']) ? sanitize_text_field($_GET['wc_modena_admin_nonce']) : null;
        ...
        if (!empty($_GET['error'])) {
            ...
            $this->logger->error(sprintf('%s: returned back from Autoconfigure flow with error: %s', __METHOD__,
                $error_message));

            $this->_redirect_with_messages(__('Sorry, Easy Setup encountered an error.  Please try again.'),
                $gateway_id);
        }
        ...
        foreach (array('client_id', 'client_secret', 'environment') as $param) {
            if (empty($_GET[$param])) {
                $this->logger->error(sprintf('%s: returned back from Autoconfigure flow but missing parameter %s',
                    __METHOD__, $param));
        ...
        foreach ($gateways as $gateway) {
            ...
            $environment = isset($_GET['environment']) ? sanitize_text_field($_GET['environment']) : 'sandbox';
