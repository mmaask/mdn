...
abstract class Modena_Base_Payment extends WC_Payment_Gateway {
  ...
  public function __construct() {
    ...
    $this->modena = new Modena(
       $this->client_id, $this->client_secret, $userAgent, $this->is_test_mode);
  ...
  public function redirect_to_modena() {
    ...
    $application = new ModenaApplication(
       $maturityInMonths, $selectedOption, strval($order->get_id()), $order->get_total(), $orderItems, $customer, date("Y-m-d\TH:i:s.u\Z"), get_woocommerce_currency());
    $request = new ModenaRequest(
       $application, sprintf('%s/wc-api/modena_response_%s', HomeUrl::baseOnly(), $this->id), sprintf('%s/wc-api/modena_cancel_%s', HomeUrl::baseOnly(), $this->id), sprintf('%s/wc-api/modena_async_response_%s', HomeUrl::baseOnly(), $this->id));
    try {
  ...
  private function create_redirect_url($orderId, $maturityInMonths, $selectedOption) {
    ...
    return sprintf(
       '%s/wc-api/redirect_to_modena_%s?id=%d&maturityInMonths=%d&selectedOption=%s', HomeUrl::baseOnly(), $this->id, $orderId, $maturityInMonths, $selectedOption);
  }
