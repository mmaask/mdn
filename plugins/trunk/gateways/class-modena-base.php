...
abstract class Modena_Base_Payment extends WC_Payment_Gateway {
  ...
  public function __construct() {
    ...
    $userAgent                       = sprintf('ModenaWoocommerce/%s WooCommerce/%s WordPress/%s',
                                               self::PLUGIN_VERSION,
    ...
    add_filter('woocommerce_available_payment_gateways', array($this, 'adjust_modena_cart_based_payments'));
    add_filter('woocommerce_available_payment_gateways', array($this, 'hide_payment_method_if_payment_is_disabled_in_settings'));
  ...
  public function init_form_fields() {
    $this->form_fields = [
       ...
       'payment_button_max_height'    => [
          ...
          'default'     => 30,
          'desc_tip'    => false,],
       'cart_sum_restriction_enabled' => [
          'title'       => sprintf(__('%s Payment Gateway Cart Restriction', 'modena'), $this->get_method_title()),
          'type'        => 'checkbox',
          'description' => 'Hide payment method in checkout if cart is lower than sum.  Default is disabled.',
          'default'     => 'no',],
       'cart_sum_restriction'         => [
          'title'       => __('Cart Restriction Sum', 'modena'),
          'type'        => 'number',
          'description' => __('Hide payment method if cart total sum is lower than this. Default is 100.', 'modena'),
          'default'     => 100,
          'desc_tip'    => false,],];
  }
  ...
  public function modena_response() {
    ...
    try {
      ...
      if ($applicationStatus !== 'SUCCESS' || !$order || !$this->validate_modena_response($modenaResponse)) {
        $this->logger->error('Order number ' . wc_get_order($modenaResponse->getOrderId()) . '. Invalid application status, expected: SUCCESS | received: ' . $applicationStatus) .
        '. Modena response is invalid: ' .
        json_encode($modenaResponse);
      ...
      if ($order->get_payment_method() === $this->id) {
        if ($order->needs_payment()) {
          ...
          if ($order->get_payment_method() === 'modena_direct') {
            $order->add_order_note(sprintf(__('Order paid by %s via %s.', 'modena'), $this->method_title, $order->get_meta(self::MODENA_SELECTED_METHOD_KEY)));
          }
          else {
  ...
  public function modena_cancel() {
    ...
    if (!$this->validate_modena_response($modenaResponse)) {
      $this->logger->error('Order number ' . wc_get_order($modenaResponse->getOrderId()) . '. Unable to confirm order for customer because of service restrictions: ' . json_encode($modenaResponse));
      wp_redirect($this->site_url);
      exit;
    }
    try {
      ...
      if ($payment_method === $this->id) {
        ...
        $this->logger->error(sprintf('Order number ' .
                                     wc_get_order($modenaResponse->getOrderId()) .
  ...
  public function modena_async_response() {
    ...
    try {
      ...
      $this->logger->error('Order number ' . wc_get_order($modenaResponse->getOrderId()) . '. \n\nException occurred in payment response: ' . $e->getMessage());
      $this->logger->error($e->getTraceAsString());
      exit;
    }
  }
  ...
  public function process_payment($order_id) {
    ...
    $orderItems = [];
    $customer   = new ModenaCustomer($order->get_billing_first_name(), $order->get_billing_last_name(), $order->get_billing_email(), $order->get_billing_phone(), join(', ', [
       $order->get_billing_address_1(),
       $order->get_billing_address_2(),
       $order->get_billing_city(),
       $order->get_billing_state(),]));
    foreach ($order->get_items(['line_item', 'fee', 'shipping']) as $orderItem) {
      $orderItems[] = new ModenaOrderItem($orderItem->get_name(), $orderItem->get_total() + $orderItem->get_total_tax(), $orderItem->get_quantity(), get_woocommerce_currency());
    }

    $selectedOption = isset($_POST['modena_selected_payment_method']) ? sanitize_text_field($_POST['modena_selected_payment_method']) : null;

    $application = new ModenaApplication($selectedOption, strval($order->get_id()), $order->get_total(), $orderItems, $customer, date("Y-m-d\TH:i:s.u\Z"), get_woocommerce_currency());
    $request     =
       new ModenaRequest($application,
  ...
  public function get_icon() {
    ...
    $icon       =
       ...
       '" title="' .
       esc_attr($this->get_icon_title()) .
       '" alt="' .
       esc_attr($this->get_icon_alt()) .
       '" style="max-height:' .
       strval($this->payment_button_max_height) .
       'px;' .
       $marginLeft .
       '"/>';
  ...
  public function initialize_variables_with_translations($id) {
    ...
    $translations = Modena_Translations::get_values_to_initialize_objects($id);  //pass the id to the function and store the result in $translations
  ...
  public function adjust_modena_cart_based_payments($available_gateways) {
    if (!is_checkout()) {
      return $available_gateways;
    }
  ...
  public function hide_payment_method_if_payment_is_disabled_in_settings($available_gateways) {
    if (get_option('modena_payments_enabled') == 'no') {
      unset($available_gateways[$this->id]);
    }
